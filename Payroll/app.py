...
def create_default_admin():
    ...
    if User.query.filter_by(role='manager').count() == 0:
        ...
        print("Default manager user created: username='manager', password='manager'")
        
    # Default Supervisor
    if User.query.filter_by(role='supervisor').count() == 0:
        supervisor = User(
            username='supervisor',
            email='supervisor@matchbox.local',
            full_name='System Supervisor',
            role='supervisor'
        )
        supervisor.set_password('supervisor')
        db.session.add(supervisor)
        print("Default supervisor user created: username='supervisor', password='supervisor'")
