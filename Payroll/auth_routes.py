...
def create_user():
    ...
    if request.method == 'POST':
        ...
        # Only allow specified roles (not admin via this route)
        if role not in ['manager', 'supervisor', 'operator', 'viewer']:
            role = 'operator'
...
def change_user_role(user_id):
    ...
    if new_role not in ['admin', 'manager', 'supervisor', 'operator', 'viewer']:
